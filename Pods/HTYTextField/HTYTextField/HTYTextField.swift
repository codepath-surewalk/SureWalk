...
open class HTYTextField: UITextField {
    ...
    required public init?(coder aDecoder: NSCoder) {
        ...
        fakePlaceholderLabel.textColor = UIColor(colorLiteralRed: 198/255.0, green: 148/255.0, blue: 67/255.0, alpha: 1.0)
        ...
        rightPlaceholderLabel.text = rightPlaceholder
        rightPlaceholderLabel.textColor = UIColor(colorLiteralRed: 198/255.0, green: 148/255.0, blue: 67/255.0, alpha: 1.0)
    ...
    open func didBeginEditing(_ notification: Notification) {
        placeholder = nil
        if !subviews.contains(fakePlaceholderLabel) {
            addSubview(fakePlaceholderLabel)
        }
        
        if notification.object is HTYTextField{
            if notification.object as! HTYTextField === self {
                UIView.animate(withDuration: 0.7, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0, options: .curveEaseIn, animations: { () -> Void in
